...
Thus we can ask, what is the probabilty that a new spillover event will lead to an ever larger outbreak than what has been observed during past outbreaks? In other words, have large outbreaks been avoided up to the current time point simply by chance? The functions in this package can help provide quantitative answers to these questions.
...
For example, from the estimates above with R = 0.7, k = 0.08 applied to all cases, what is the probability that a singles spillover event will result in an outbreak of 100 or more cases, i.e. more than twice as big as the previous worst outbreak? (I am confused about the "worst outbreak". You also mention this in the intro. The worst outbreak ever recorded is going to vary by pathogen. Is there a way to make the "worst outbreak" a random variable? Why 100 cases? Or can you clarify how this is accounted for in your model?)
